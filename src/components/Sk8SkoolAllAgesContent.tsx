...
interface Sk8SkoolAllAgesContentProps {
  /**
   * Hub this page was entered from. All Ages is listed on both the Adults and
   * the Kids Space Sk8 Skool hubs, so a single hardcoded back link is always
   * wrong for one of them - each space renders this content at its own route
   * and passes its own hub here.
   */
  backHref: string;
}

export default function Sk8SkoolAllAgesContent({ backHref }: Sk8SkoolAllAgesContentProps) {
  return (
    <>
      <Navbar />
