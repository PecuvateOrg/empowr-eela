...
**✅ 2026-08-29 — the Sk8 Skool hub work merged to `main` (local only, not pushed); the Members-link cutover deliberately kept separate.** `feat/sk8-skool-merge`'s hub build (Kids Space Sk8 Skool hub, 6-gap KB audit, Synkron8/Beginners Foundation/Skate Jam detail pages) is now on `main`, plus the "Beginners Foundation" singular-copy commit cherry-picked in on its own (display strings only, no link dependency). **The two Members-repoint commits (`5f2e064`, `0cb9387`) were deliberately NOT merged** — the user's call, since the Members side of the cutover is still being worked out. `feat/eela-booking-cutover` was rebuilt to contain just those two commits, rebased on the new `main`, ready to merge whenever Members is ready — **don't re-derive the Members URLs from scratch, just merge that branch**. `feature/skate-jam-page` and the old `feat/sk8-skool-merge` are superseded by the hub merge; don't merge them again. Conflicts (main had independently gained Turnstile, Private Bookings, chat-widget work) resolved file-by-file: `EnquiryModal.tsx` kept `main`'s Turnstile-enabled superset, `kids-programmes.tsx` kept the incoming branch's Roller Skate Events card, `DEVLOG.md`/this file reconciled chronologically. A first attempt merged everything (hub + cutover) then reverted `links.ts` back to Wix in a follow-up commit — superseded by this cleaner split, don't repeat that approach.
...
- **Members-side booking cutover** — `feat/eela-booking-cutover` now holds just the 2 Members-repoint commits, rebased on current `main`, ready to merge whenever Members is settled. See Current Phase above — don't re-derive the Members URLs, just merge that branch.
- **`kids-space/sk8-skool/kidz/page.tsx`'s £30/mo Membership CTA points at `LINKS.kidzSpace`** (generic Wix page) instead of a day-specific link, unlike every other detail page where Pay-As-You-Go and Membership share one link. Pre-existing on `feat/sk8-skool-merge`, found during 2026-08-29 review, not fixed.
- **Resolve the Kids Space Roller Skate Events link target** (`LINKS.rollerSkateEvents` — currently points at the adult-only 15+ page, but the Kids Space card copy is family-oriented) — flagged 2026-08-25, awaiting the user's call.
- **Restore public "All Ages Roller Disco" content** (the recurring family session, distinct from the Roller Disco Birthday Party private booking above) — Members has an offering for it (inactive) but EELA has no page linking to it; see `DEVLOG.md` 2026-07-31 entry and the 2026-08-26 entry's outstanding-work note.
