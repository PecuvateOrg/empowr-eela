...
- **Members-side booking cutover** — `feat/eela-booking-cutover` holds just the 2 Members-repoint commits, rebased on current `main`, pushed to origin as a backup, ready to merge whenever Members is settled. See Current Phase above — don't re-derive the Members URLs, just merge that branch.
